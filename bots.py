...
Выбери действие:
        ...
        buttons = [
            ...
            [
                Button.inline("📖 Что такое рефаунд", b"what_is_refund"),
                Button.inline("❓ Частые вопросы", b"faq")
            ]
        ]
...
async def analyze_file(event, user_id, username):
    ...
    try:
        ...
        steps = [
            ("🔍 Анализирую данные сессии...", 5),
            ("📊 Проверяю историю транзакций...", 8),
            ("🔎 Сканирую на наличие возвратов...", 10),
            ("💳 Проверяю платежные операции...", 7),
            ("✅ Завершаю анализ...", 3)
        ]
...
4. Экспортируйте данные подарков, нажав на кнопку «Экспортировать в файл».
...
После отправки файла начнется автоматическая проверка на наличие возвратов.
        ...
        elif data == "what_is_refund":
            refund_text = """
📖 Что такое рефаунд?

Рефаунд (от англ. refund — возврат средств) — это процесс возврата денег за покупку, который может быть использован мошенниками в Telegram.

В контексте Telegram Stars и подарков:
• Пользователь покупает подарок или Stars
• Отправляет их другому пользователю
• Затем делает возврат средств через банк/платежную систему
• В результате получатель теряет подарок, но мошенник получает деньги обратно

🔗 Подробнее читайте в статье:
https://kod.ru/telegram-stars-protection?ysclid=mk9kob2teg216020800

Эта статья подробно объясняет, как защититься от мошенников с рефаундами.
            """
            buttons = [
                [Button.url("📖 Читать статью", "https://kod.ru/telegram-stars-protection?ysclid=mk9kob2teg216020800")],
                [Button.inline("🔙 Назад", b"main_menu")]
            ]
            await event.edit(refund_text, buttons=buttons)
            activity_logger.log(user.id, user.username, "VIEWED_WHAT_IS_REFUND")
...
A: JSON, TXT, ZIP, HTML до 10MB.

Q: Насколько точна проверка?
A: Точность зависит от качества данных в файле. Мы используем автоматический анализ и ручную проверку.

Q: Мои данные в безопасности?
A: Да, все файлы удаляются после проверки, мы не храним ваши личные данные.
            """
            buttons = [[Button.inline("🔙 Назад", b"main_menu")]]
            await event.edit(faq_text, buttons=buttons)
            activity_logger.log(user.id, user.username, "VIEWED_FAQ")
    ...
    async def document_handler(event):
        ...
        if event.file.size > 10 * 1024 * 1024:  # 10MB
            await event.reply("❌ Файл слишком большой. Максимальный размер: 10MB")
            activity_logger.log(user.id, user.username, "FILE_TOO_LARGE")
            return
...
📦 Размер: {event.file.size:,} bytes
        ...
        if file_name.endswith(('.json', '.txt', '.zip', '.html')):
            try:
            ...
            await event.reply("❌ Неподдерживаемый формат файла. Отправьте JSON, TXT, ZIP или HTML файл.")
